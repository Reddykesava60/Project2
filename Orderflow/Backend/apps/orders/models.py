...
class OrderReservation(models.Model):
    """
    Transient reservation record to lock stock BEFORE payment.
    Created when user initiates checkout.
    Expires automatically if payment not completed.
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    restaurant = models.ForeignKey(Restaurant, on_delete=models.CASCADE)
    
    # Store items snapshot: [{menu_item_id, quantity, price, name}]
    ...
    class Status(models.TextChoices):
        ACTIVE = 'active', 'Active'     # Stock locked, waiting for payment
        PAID = 'paid', 'Paid'           # Converted to real order
        EXPIRED = 'expired', 'Expired' # Timeout, stock released
    ...
    # Metadata for eventual order creation
    customer_name = models.CharField(max_length=100)
    table_number = models.CharField(max_length=20, blank=True)
    payment_method = models.CharField(max_length=10) # 'upi' or 'cash'
    is_parcel = models.BooleanField(default=False)
    spicy_level = models.CharField(max_length=20, default='normal')
