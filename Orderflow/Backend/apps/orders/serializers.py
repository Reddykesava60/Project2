...
def build_item_unavailable_error(*, menu_item: MenuItem | None, requested_quantity: int | None = None) -> dict:
    ...
    if menu_item is not None:
        ...
        if menu_item.stock_quantity is not None:
            available_quantity = int(menu_item.stock_quantity)

    payload: dict = {
        'code': 'ITEM_UNAVAILABLE',
        'message': 'This item is out of stock or no longer available.',
    }
    ...
    if requested_quantity is not None:
        payload['requested_quantity'] = int(requested_quantity)
            ...
            for item_data in items_data:
                ...
                try:
                    ...
                    raise serializers.ValidationError(
                        build_item_unavailable_error(menu_item=None, requested_quantity=quantity)
                    )

                # Availability check (backend is authority)
                if not menu_item.is_active or not menu_item.is_available:
                    raise serializers.ValidationError(
                        build_item_unavailable_error(menu_item=menu_item, requested_quantity=quantity)
                    )

                # STOCK CHECK & RESERVATION
                if menu_item.stock_quantity is not None:
                    # reserve_stock(qty) handles atomic check and update
                    if not menu_item.reserve_stock(quantity):
                        raise serializers.ValidationError(
                            build_item_unavailable_error(menu_item=menu_item, requested_quantity=quantity)
                        )
                    # No save needed here as reserve_stock saves atomically
...
class CollectPaymentSerializer(serializers.Serializer):
    ...
    def update(self, order, validated_data):
        ...
        return order
class OrderReservationCreateSerializer(serializers.Serializer):
    """Serializer for creating an order reservation (locking stock)."""
    ...
    # Extras
    is_parcel = serializers.BooleanField(required=False, default=False)
    spicy_level = serializers.ChoiceField(choices=['normal', 'medium', 'high'], required=False, default='normal')
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            # Calculate totals and prepare items snapshot
            subtotal = Decimal('0.00')
            reservation_items = []
