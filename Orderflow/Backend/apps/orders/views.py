    ...
    def get_queryset(self):
        ...
        if user.role in ['staff', 'STAFF']:
            ...
            if staff_profile and staff_profile.restaurant:
                queryset = Order.objects.filter(restaurant=staff_profile.restaurant)
                
                # Super Staff (can_collect_cash) rules:
                # - Pending (Cash only): To verify and collect payment
                # - Preparing: To manage active orders
                # - Completed: To review historical orders
                if staff_profile.can_collect_cash:
                    from django.db.models import Q
                    return queryset.filter(
                        Q(status='pending', payment_method='cash') |
                        Q(status__in=['preparing', 'completed'])
                    )
                
                # Normal Staff rules:
                # - Preparing only: To fulfil active kitchen orders
                else:
                    return queryset.filter(status='preparing')
    ...
    def collect_payment(self, request, pk=None):
        ...
        with transaction.atomic():
            for item in order.items.select_related('menu_item').all():
                if item.menu_item:
                    # Confirm sale (Total Stock -= Qty, Reserved -= Qty)
