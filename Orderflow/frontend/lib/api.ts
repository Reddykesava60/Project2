  ...
  login: (email: string, password: string) =>
    handleResponse<{ user: User; access: string; refresh: string }>(
      ...
      if (!res.success || !res.data) {
        return res as unknown as ApiResponse<{ user: User; token: string }>
      }
...
export const staffApi = {
  ...
  create: (restaurantId: string, data: any) =>
    handleResponse<StaffMember>(http.post(`/restaurants/staff/?restaurant=${restaurantId}`, { ...data, restaurant: restaurantId })),
