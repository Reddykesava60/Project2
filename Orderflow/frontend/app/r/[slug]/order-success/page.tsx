...
import { CheckCircle2, Clock, Banknote, CreditCard, QrCode, Home, Bold } from 'lucide-react';
            ...
            {isCashPayment && (
              <div className="rounded-lg bg-warning/10 p-3">
                <p className="text-sm font-medium text-warning-foreground">
                  Please pay {(order?.total || order?.total_amount) ? `$${(order.total || order.total_amount).toFixed(2)}` : ''} at the counter then only order is Conformed
                </p>
              </div>
            )}
          </CardContent>
