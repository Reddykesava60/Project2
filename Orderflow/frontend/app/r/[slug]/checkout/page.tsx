...
export default function CheckoutPage() {
  ...
  // 1. Fetch Restaurant & Menu (Polled)
  ...
  const { data: menuData } = useSWR(
    ...
    {
      refreshInterval: 3000, // Poll every 3s for critical checkout stock
      revalidateOnFocus: true
    }
  );

  // 2. Validate Stock Effect
  useEffect(() => {
    if (!menuData?.data || items.length === 0) return;

    const categories = menuData.data;
    // Flatten all items from categories
    const allMenuItems = categories.flatMap((cat: any) => cat.items);

    let newStockError = null;

    for (const cartItem of items) {
      const liveItem = allMenuItems.find((i: any) => i.id === cartItem.menu_item.id);

      if (!liveItem) {
        // Item removed from menu?
        newStockError = `Item '${cartItem.menu_item.name}' is no longer available.`;
        break;
      }

      if (!liveItem.is_available) {
        newStockError = `Item '${liveItem.name}' is currently unavailable.`;
        break;
      }

      // Check stock (available_stock returned as stock_quantity)
      // null means unlimited
      if (liveItem.stock_quantity !== null && liveItem.stock_quantity < cartItem.quantity) {
        const available = liveItem.stock_quantity;
        if (available <= 0) {
          newStockError = `Item '${liveItem.name}' is out of stock.`;
        } else {
          newStockError = `Only ${available} left for '${liveItem.name}'. Please reduce quantity.`;
        }
        break;
      }
    }
  ...
  }, [items, menuData]); // Re-run when cart or polled menu data changes

  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const cartSignature = useMemo(
    () => items.map((i) => `${i.menu_item.id}:${i.quantity}`).join('|'),
    [items]
  );
  const [blockedSignature, setBlockedSignature] = useState<string | null>(null);
  const isStockBlocked = blockedSignature !== null && blockedSignature === cartSignature;

  // Block submission if stock error exists
  const isSubmissionBlocked = isStockBlocked || (stockError !== null);

  // Update local error state if stock error changes (override)
  useEffect(() => {
    if (stockError) {
      setError(stockError);
    } else if (error === 'Item out of stock') { // simple check to clear auto-error
      setError(null);
    }
  }, [stockError]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentMethod === 'upi') {
        ...
        // Step B: Reservation & Initiate Payment
        // 1. Create Reservation (Locks Stock)
        const reservation = await publicService.createReservation(slug, {
          items: orderItems,
          customer_name: customerName.trim(),
          table_number: useCartStore.getState().tableNumber,
          is_parcel: useCartStore.getState().isParcel,
        ...
        // 2. Initiate Payment using Reservation ID
        const initData = await publicService.initiateUPIPayment(slug, reservation.reservation_id);
        ...
        const options = {
          ...
          handler: async function (response: any) {
            try {
              ...
              if (verErr?.code === 'ITEM_UNAVAILABLE' || verErr?.code === 'INSUFFICIENT_STOCK') {
                const base = '❌ This item is out of stock or no longer available.'
                const details = `${verErr.item_name ? `\nItem: ${verErr.item_name}` : ''}${verErr.available_quantity !== undefined && verErr.available_quantity !== null
                  ? `\nAvailable: ${verErr.available_quantity}`
                  : ''
                  }`
                setError(`${base}${details}`)
                setBlockedSignature(cartSignature)
              } else {
      ...
      if ((err as any).code === 'ITEM_UNAVAILABLE' || (err as any).code === 'INSUFFICIENT_STOCK') {
        const base = '❌ This item is out of stock or no longer available.'
        const details = `${err.item_name ? `\nItem: ${err.item_name}` : ''}${err.available_quantity !== undefined && err.available_quantity !== null
          ? `\nAvailable: ${err.available_quantity}`
          : ''
          }`
        setError(`${base}${details}`);
        setBlockedSignature(cartSignature);
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-background p-4 pb-safe">
        <div className="mx-auto max-w-2xl">
          <Button
            ...
            disabled={isSubmitting || isSubmissionBlocked || items.length === 0 || !customerName.trim() || !privacyAccepted}
