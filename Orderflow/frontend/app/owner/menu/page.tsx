...
export default function OwnerMenuPage() {
  ...
  const handleUpdateStockInline = async (item: MenuItem, rawValue: string) => {
    const trimmed = rawValue.trim();
    const nextStock = trimmed === '' ? null : Number(trimmed);
    const currentStock = item.stock_quantity ?? null;

    if (Number.isNaN(nextStock as number)) {
      mutate();
      return;
    }
    ...
    const response = await menuApi.updateItem(item.id, {
        ...
        {isLoading ? (
          ...
          <ErrorState
            ...
            message={
              fetchError?.message ||
              menuData?.error ||
              "Unable to fetch menu data. Please check your connection."
            }
            onRetry={() => mutate()}
          />
        ) : categories.length === 0 ? (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2">
                            ...
                            <Input
                              id={`stock-${item.id}`}
                              key={`${item.id}-${item.stock_quantity ?? 'null'}`}
                              type="number"
                              min={0}
                              placeholder="∞"
                              defaultValue={item.stock_quantity === null || item.stock_quantity === undefined ? '' : String(item.stock_quantity)}
                              onBlur={(e) => handleUpdateStockInline(item, e.target.value)}
                              className="h-9 w-20"
                            />
                            {item.available_stock !== undefined && item.stock_quantity !== null && (
