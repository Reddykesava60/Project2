...
export function Sidebar() {
  ...
  // Base staff nav
  staffNav.push({ label: 'Active Orders', href: '/staff/orders', icon: <ShoppingBag className="w-5 h-5" /> })

  // Permission-based nav
  if (user?.can_collect_cash) {
    staffNav.push({ label: 'Create Cash Order', href: '/staff/create-order', icon: <Package className="w-5 h-5" /> })
  }

  if (user?.can_manage_stock) {
    staffNav.push({ label: 'Stock Control', href: '/staff/stock', icon: <Package className="w-5 h-5" /> })
  }
